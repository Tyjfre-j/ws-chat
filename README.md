...
- Live connection status in the client UI: Connecting, Connected, and Disconnected
- Automatic reconnection with exponential backoff: 1s, 2s, 4s, and so on, capped at 30s
- Retry interruption with `Esc`
- Graceful handling of malformed input, empty fields, oversized messages, duplicate usernames, and unexpected disconnects
...
| Type               | Payload                 | When                              |
| ------------------ | ----------------------- | --------------------------------- |
| `set_username`     | `{ "username": "..." }` | Proposing a username              |
| `confirm_username` | `{ "confirmed": true }` | Answering the confirmation prompt |
| `join_room`        | `{ "room": "..." }`     | Choosing a room                   |
| `chat_message`     | `{ "message": "..." }`  | Sending a chat message            |
...
| `confirm_username`          | `{ "username": "..." }`                   | Echoing the proposed username for confirmation          |
| `room_list`                 | `{ "rooms": ["..."] }`                    | After the username is confirmed                         |
...
- **Control frames are separate from application messages:** Ping, pong, and other non-text WebSocket frames are ignored by the application rather than parsed as JSON. The server and client do not implement a separate application-level heartbeat.
